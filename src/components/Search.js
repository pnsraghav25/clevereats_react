...
const Search = () => {
    ...
    const [status, setStatus] = useState('Suggested: Lays, Oreo, Coke');
        ...
        if (name) {
            ...
            fetch(`https://api.edamam.com/api/food-database/v2/parser?app_id=5a62c25f&app_key=d3784bbd33ff5dbf82bf4e262240870a&brand=${name}`)
                ...
                .then(data => {
                    ...
                    setResults(hints.map((e, i) => (
                        <div className="w-72 bg-white shadow-md rounded-xl duration-50005 hover:shadow-xl" key={i}>
                            <div className="w-72 px-4 py-3">
                                <span className="text-lg font-bold text-black block capitalize mb-3">{e.food.label}</span>
    ...
    return (
        <>
            <div className="flex flex-col overflow-auto" style={{ flexGrow: 1 }}>
                <div className="bg-indigo-900 h-48 flex items-center justify-center ">
                    <span className="font-semibold text-2xl text-white">Search for a Product</span>
                </div>
                <div className="flex w-4/4 justify-center px-8">
                    <input className="rounded-lg shadow-lg w-1/3 px-6 py-4 -mt-8 bg-white" placeholder="Enter name of product" value={name} onChange={e => setName(e.target.value)} />
                </div>
                <div className="flex flex-col items-center">
                    <div className="flex items-center mt-12">
                        <button className="flex bg-indigo-800/10 border border-indigo-400 py-2 text-center justify-center items-center rounded-lg px-10" onClick={call}>
                            <span className="font-semibold text-sm">Search</span>
                        </button>
                    </div>
                    <div className='mt-8'>
                        <span className="my-8 text-center font-semibold text-lg">{status}</span>
                    </div>
                </div>
                <div className="grid grid-cols-3 justify-center my-12 gap-12 self-center">{results}</div>
                {showModal && (
